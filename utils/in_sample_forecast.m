% adapted for Dynare version 5. February 2008. Michel Juillard
% This code generate charts with in sample forecast for a model.
function in_sample_forecast(fy, F, Udecomp, ndiffuse, freq, nahead, exo_ord, ...
...
for ivar=1:nvar
    ...
    svar = cellstr(deblank(M_.endo_names(order_var(ivar))));

    if isempty(strmatch(svar{1},var_list,'exact'))
        continue
    end
  tit = strcat(deblank(svar)," ",num2str(nahead)," periods ","ahead (",num2str(id),")");
  figure('Name', tit, 'Visible', 'off');
%  eval(['y=' deblank(svar) ';']);
%   y = oo_.SmoothedVariables.(deblank(svar{1}))+oo_.dr.ys(order_var(ivar));
  y = oo_.SmoothedVariables.(deblank(svar{1}));
  % calculate th_rmse
  ff = F(order_var(ivar),nahead,:);
...
%   subplot(1,1,1);
  ...
  hold on
  for t = rng
    ycast = reshape(fy(order_var(ivar),1:nahead,t+1), nahead, 1);
    ycast = [y(t); ycast];
...
%   set(gca,'XTickLabel',xlabels);
  % print
  print('-dpdf',['charts/' M_.dname '_in_sample_forecast_' deblank(svar{1}) '_' num2str(nahead) '.pdf']);
%   print('-depsc2',[M_.dname '_dynamic_fcast_' deblank(svar{1}) '_' num2str(nahead) '_' ...
%                    num2str(id) '.eps']);
end
...
for i=1:size(strin,1)
  j = strfind(strin(i,:), '_');
  if isempty(j)
    strout = strcat(strout, strin{i});
  else
    strout = strcat(strout, " ", extractAfter(strin{i}, j{1}(1)));
  end
end
